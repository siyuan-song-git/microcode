...
int main(void)
{
	...
	for (i=0; i<7; i++)
			*(volatile unsigned long *)(0x2400018UL+i*8) = (unsigned long)&_secondary_cpu_start >> 2;
	*(volatile unsigned int *)0x2400008 = 0x0;			//release all cpu


	cpu_id = get_cpu_id();
	write_cpu_wait_flag(cpu_id);
	while(1);
...
void secondary_cpu_entry(void)
{
	...
	cpu_id = get_cpu_id();
	write_cpu_wait_flag(cpu_id);
	cpu_mask = read_cpu_wait_flag();
	//mprintf("cpu_mask: 0x%x\r\n",cpu_mask);
	if ((cpu_mask + 1) == (1 << CPU_MAX)) {
		mprintf("send SGI0 to all cpu by nonbroadcast!\r\n");
		write_ICC_SGI1R_EL1(cpu_wait_flag);
	}
}